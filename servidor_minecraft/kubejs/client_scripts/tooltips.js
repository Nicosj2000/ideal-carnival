const toolTips = [
  ...
  ["immersiveengineering:seed", "tooltip.hemp_seeds"],
...
Object.entries(oreMap).forEach(([ore, coords]) => {
  itemsToTooltip.push({
    ...
    summary: [
      [
        Text.of(Component.translate("tooltip.ore.y_level")).darkGray(),
...
ItemEvents.tooltip((tooltip) => {
  itemsToTooltip.forEach((tooltipItem) => {
    tooltip.addAdvanced(tooltipItem.item, (item, advanced, text) => {
      ...
      if (!tooltip.shift) {
        text.add(1, [
          Text.of(Component.translate("tooltip.shift.1")).darkGray(),
          Text.of("[").gray(), Text.of(Component.keybind("key.sneak")).gray(), Text.of("]").gray(),
          Text.of(Component.translate("tooltip.shift.2")).darkGray(),
        ]);
        // When Holding Shift
      } else {
